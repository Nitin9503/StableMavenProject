...
public class Reusemethod extends Loginapp
{
	...
	SplitReceiptPage splitReceiptPage =new SplitReceiptPage();
	PayingPaymentPage payingPaymentPage = new PayingPaymentPage();
	 HomePage homePage = new HomePage(driver); 
	 GuestPage guestPage = new GuestPage(driver);
	 OrderPage orderPage = new OrderPage(driver);
	 TransactionOrderPage transactionOrderPage = new TransactionOrderPage(driver);*/
   ...
   public void addGuestToOrder(){
       ...
       Assert.assertEquals(guestNameOrder, "Guest 1", "Order not created after creating order (Guest is not added to order)");
...
/*				List<WebElement>totalItemAddedToOrder = driver.findElements(By.id("com.torenzo.torenzocafe:id/guest_orderd_item_recycler_view"));
	...
	public void Payment() throws IOException, InterruptedException
	{
		...
		Assert.assertEquals(puff.size(), guest.size(), "Splited receipt as per guest and receipts are not splited as per guest (Failed)");
	    driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		Thread.sleep(4000);
 ...
 public void DbconnReceiptdata(){
	 ...
	 try{  
		 ...
		 ResultSet rd=stmt.executeQuery("select * from finaldbnew.drawertransaction where InvoiceNumber='"+invoice_no+"';");  
		 while(rd.next())
